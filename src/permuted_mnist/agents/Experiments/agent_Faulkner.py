...
class Agent:
    ...
    def __init__(self, output_dim: int = 10, seed: int = None,
                 ...
                 verbose: bool = True):
        """
        Initialize the MLP agent with training improvements inspired by mlp.py.
        New features: label smoothing, warmup+cosine schedule, early stopping,
        optional per-sample L2 unit-norm, and soft reset across tasks.
        """
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray):
        ...
        if self.use_label_smoothing:
            criterion = SmoothCE(self.smoothing_eps, num_classes=self.output_dim)
        else:
        ...
        # Scheduler
        scheduler = WarmupCosine(opt, total_epochs=self.epochs, warmup_epochs=self.warmup_epochs) if self.use_warmup_cosine else None
